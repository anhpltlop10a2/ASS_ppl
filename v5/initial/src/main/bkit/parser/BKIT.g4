...
func_declare : FUNCTION COLON ID (PARAMETER COLON parameter_name)? (BODY COLON)(var_declare* stm_list)(ENDBODY DOT);  
stm_list:  stm* ; 
stm     : assign_stm
        | function_call_stm
        | if_stm 
...
for_stm: FOR LP ID '=' INT_LITERAL COMMA  exp COMMA exp RP DO (stm_list) ENDFOR DOT ;
...
fragment INT_PART: DIGIT_EX0 (DIGIT)*| '0'; 
...
ARRAY_LITERAL:	LB  ' '* (' '*(ARRAY_LITERAL | LITERAL) ' '* (' '* COMMA ' '* (LITERAL | ARRAY_LITERAL) ' '* )* ' '*)?  ' '* RB;
//ARRAY_LITERAL:LB ((ARRAY_LITERAL | LITERAL) ( COMMA  (LITERAL | ARRAY_LITERAL)  )* )?  RB;
